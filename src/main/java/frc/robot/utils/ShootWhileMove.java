...
public class ShootWhileMove {
  ...
  public static ShootingCommand newShooting(Translation3d target, Translation3d botPose, Translation2d botVel){
    double g=9.8065;
	    double v=11.3;
      var pbmt=target.minus(botPose);
	    ...
	    for(int i=0;i<20;i++){
	        ...
	        double ex=vx*t-px;
	        double ey=vy*t-py;
	        double dtdts=(vground-(v*vground*Math.sin(theta_s)/Math.sqrt(vz*vz-2*g*pz)))/g;
