...
public class SpeakerShoot extends Command {
  ...
  public void execute() {
    ...
    if (Double.isNaN(cmd.shooterAngle.getRadians())) {
      ...
      Commands.runOnce(()->driver.setRumble(GenericHID.RumbleType.kLeftRumble,0.5)).andThen(Commands.waitSeconds(0.04)).andThen(()->driver.setRumble(GenericHID.RumbleType.kLeftRumble,0)).repeatedly().withTimeout(0.5);
      cancel();
    } else {
