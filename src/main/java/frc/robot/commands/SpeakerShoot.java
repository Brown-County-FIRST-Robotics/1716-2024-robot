...
public class SpeakerShoot extends Command {
  ...
  public void execute() {
    ...
    Rotation2d angleToSpeaker=FieldConstants.getSpeaker().toTranslation2d().minus(pos.getTranslation()).getAngle().minus(Rotation2d.fromDegrees(180)).unaryMinus();
    var cmd =
        ShootWhileMove.calcCommandWithKinematics(
            pos.getTranslation(),
            FieldConstants.getSpeaker().plus(new Translation3d(0,angleToSpeaker.getDegrees()/200,0)),
            ShootWhileMove.getFieldRelativeSpeeds(
                drive.getVelocity(), drive.getPosition().getRotation()),
            kinematics);
    shooter.setSpeed(9.88); // Max speed
    Logger.recordOutput("SHOOTINGTO",new Pose3d(FieldConstants.getSpeaker().plus(new Translation3d(0,angleToSpeaker.getDegrees()/200,0)), new Rotation3d()));
    rotationCommander.accept(Optional.of(cmd.botAngle));
    if (Double.isNaN(cmd.shooterAngle.getRadians())) {
      XboxController driver=new XboxController(0);
      Commands.runOnce(()->driver.setRumble(GenericHID.RumbleType.kLeftRumble,0.5)).andThen(Commands.waitSeconds(0.04)).andThen(()->driver.setRumble(GenericHID.RumbleType.kLeftRumble,0)).repeatedly().withTimeout(0.5).schedule();
      cancel();
    } else {
