...
public class Climber extends SubsystemBase {
  ...
  public void periodic() {
    ...
    checkBottomSensors();
    if (downLocked[0] && (inputs.leftPosition > sensorOffsetFromBottom || inputs.leftTopSensor)) {
      climberIO.setMotorEncoderPosition(false, 1000);
      downLocked[0] = false; 
    }
    if (downLocked[1] && (inputs.rightPosition > sensorOffsetFromBottom || inputs.rightTopSensor)) {
      climberIO.setMotorEncoderPosition(true, 1000);
      downLocked[1] = false; 
    }
  }
  ...
  public void setMotors(double left, double right) {
    if (left < 0 && (inputs.leftPosition < -sensorOffsetFromBottom || downLocked[0])) {
      left = 0;
    } else if (left > 0 && inputs.leftTopSensor) {
      left = 0;
    }
    if (right < 0 && (inputs.rightPosition < -sensorOffsetFromBottom || downLocked[1])) {
      right = 0;
