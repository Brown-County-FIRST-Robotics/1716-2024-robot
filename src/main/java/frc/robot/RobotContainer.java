...
public class RobotContainer {
    ...
    secondController
        ...
        .whileTrue(
            Commands.run(
                ...
                arm))
        .onFalse(Commands.runOnce(arm::commandNeutral, arm)).whileTrue(Commands.run(()->{teleopDrive.setCustomRotation(Optional.of(Rotation2d.fromDegrees(90)));}))
  ...
  public Command getAutonomousCommand() {
    ...
    return new SpeakerShoot(driveSys,arm,rt::setCustomRotation,shooter, secondController.getHID()).raceWith(rt.repeatedly()).andThen(new RotateTo(driveSys,Rotation2d.fromRotations(0.5))).andThen(new HolonomicTrajectoryFollower(driveSys,()->{
      ...
      return TrajectoryGenerator.generateTrajectory(new Pose2d(driveSys.getPosition().getTranslation(),startRot), List.of(),new Pose2d(target,Rotation2d.fromRotations(0.5)),conf);
    },Rotation2d.fromRotations(0.5)).repeatedly().raceWith(Intake.fromFloor(shooter,arm,secondController.getHID()))).andThen(new RotateTo(driveSys,Rotation2d.fromRotations(0))).andThen(Commands.waitSeconds(1)).andThen(new SpeakerShoot(driveSys,arm,rt2::setCustomRotation,shooter, secondController.getHID()).raceWith(rt2.repeatedly()));
  }
}
